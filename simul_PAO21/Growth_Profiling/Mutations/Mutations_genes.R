...
iPau21_react_2 <- iPau21_react
iPau21_react_2$rule <- gsub("\\|", "or", iPau21_react$rule)
iPau21_react_2$rule <- gsub("&", "and", iPau21_react$rule)
write.table(iPau21_react_2, file = "iPau21Caz5d_react.tsv", sep = "\t", 
            row.names = FALSE ,quote = FALSE, na = "")

### Mutation for ORFN gene
G_PA14_23460 <- FALSE
...
for (reaction in 1:nrow(iPau21_react)) {
  ...
  if (rule == "") {
    ...
    if (eval(parse(text = rule)) == TRUE) {
      ...
      iPau21_react[reaction,]$uppbnd <- 0
      cat(info_reaction$abbreviation, "\t", info_reaction$name, "\t", rule, "\n",
          file = "reactions_inactivated_orfn.txt", append = TRUE)
    }
  }
}
...
iPau21_react$rule <- gsub("\\|", "or", iPau21_react$rule)
iPau21_react$rule <- gsub("&", "and", iPau21_react$rule)
write.table(iPau21_react, file = "iPau21Caz5d_orfn_react.tsv", sep = "\t", 
            row.names = FALSE ,quote = FALSE, na = "")
